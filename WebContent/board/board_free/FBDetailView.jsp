							...
							<td>
								<div class="comments-write">	
									<form id="comments_form" action="<%=YesForm%>" method="POST" onsubmit="return commentCheck()">
										<input type="hidden" name="command" value="FBCWrite">
										<input type="hidden" name="no" value="${bean.no}">
									...
									<tr class="comment-list">
										...
										<td class="comment-button" align="right" width="20%">
											...
											<form id="comments_delete_form" action="<%=YesForm%>" method="GET" onsubmit="return deleteComment()">
												<input type="hidden" name="command" value="FBCDelete">
...
<script>
	...
	function deletePost() {
		if (confirm('게시물을 삭제 하시겠습니까?')) {
			location.href = "<%=NoForm%>FBDelete&no=${bean.no}&${requestScope.parameters}";
		}
	}
	...
	function modifyPost() {
		location.href = "<%=NoForm%>FBModify&no=${bean.no}&${requestScope.parameters}";
	}
	...
	function replyPost() {
		location.href = "<%=NoForm%>FBReply&no=${bean.no}&${requestScope.parameters}&groupno=${bean.group_no}&orderno=${bean.order_no}&depth=${bean.depth}"
	}
